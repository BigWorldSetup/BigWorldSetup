  ...
  'Else
    ...
    If objFSO.FolderExists(".git") Then
      'WScript.Echo "Folder exists."
        wshShell.Run "DEL .git\index.lock", 1, True
        wshShell.Run """BiG World Setup\Tools\Git\bin\git.exe"" remote update", 1, True
        wshShell.Run "%comspec% /k ""BiG World Setup\Tools\Git\bin\git.exe"" fetch & pause & exit", 1, True
        wshShell.Run "%comspec% /k ""BiG World Setup\Tools\Git\bin\git.exe"" reset --hard & pause & exit", 1, True
        wshShell.Run "%comspec% /k ""BiG World Setup\Tools\Git\bin\git.exe"" pull --rebase & pause & exit", 1, True
