...
Func _Backup_Create($p_Game, $p_Message)
	...
	For $f=1 to $Files[0]
		...
		If StringRegExp($Files[$f], '(?i)\A(Big World Backup|BWS-Downloads|App|cache|cd\d|data|ereg|mplayer|script compiler|debugs)\z') Then ContinueLoop; don't copy useless or untouched folders
		If $IsDir Then
	...
	Local $Fix[6] = ['portraits', 'save', 'mpsave', 'App', 'Big World Backup', 'BWS-Downloads']
	For $f in $Fix
...
; ---------------------------------------------------------------------------------------------
	...
	For $b=1 to $BakFiles[0]
		If StringRegExp($BakFiles[$b], '(?i)\A(CD\d|BWS-Downloads|Big World Backup|App|Portraits)\z') Then ContinueLoop; do >>not<< remove useful or essential folders
...
; ---------------------------------------------------------------------------------------------
		...
		For $f=1 to $Files[0]
			If _IniRead($ReadSection, $Files[$f], -1) <> -1 Then ContinueLoop; includes portraits, save, mpsave, Big World Setup + its vbs, Big World Backup, BWS-Downloads
			$FileList[0][0] +=1
...
; ---------------------------------------------------------------------------------------------
	...
	For $b=1 to $BakFiles[0]
		If StringRegExp($BakFiles[$b], '(?i)\A(BWS-Downloads|Big World Backup|App|mpsave|Portraits|save|BWS_Backup.ini)\z') Then ContinueLoop; do >>not<< copy old content
