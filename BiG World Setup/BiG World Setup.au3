...
Global $g_BG1Dir, $g_BG2Dir, $g_BG1EEDIR, $g_BG2EEDIR, $g_IWD1Dir, $g_IWD2Dir, $g_PSTDir, $g_RemovedDir, $g_BackupDir, $g_LogFile = $g_LogDir & '\BiG World Debug.txt'
...
Func _GetCurrent()
	...
	$Current[0][0] = 0
	EndIf
	If $g_Flags[21] = '' Then Return $Current; BWS will not install BGEE-mods and EET
	$Num = StringRegExpReplace($g_Flags[14], '(?i)\ABG|EE\z', '')
	Local $Return[$Current[0][0] + 1][2]
	For $c = 1 To $Current[0][0]
		If StringRegExp($g_Flags[20 + $Num], '(?i)(\A|\x7c)' & $Current[$c][0] & '(\z|\x7c)') Then; trim selection to BGEE/BG2EE mods only
			$Return[0][0] += 1
