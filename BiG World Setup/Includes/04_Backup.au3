...
; ---------------------------------------------------------------------------------------------
; Creates a multi-install
; ---------------------------------------------------------------------------------------------
Func _Backup_CreateMultiInstall()
	Local $Message = IniReadSection($g_TRAIni, 'BA-MultiInstall')
	Local $FMessage = IniReadSection($g_TRAIni, 'BA-FileAction')
	_PrintDebug('+' & @ScriptLineNumber & ' Calling Au3CleanInst')
	...
	['IWD2', 'IWD2', 'IWD2', 'IWD2'], ['PST', 'Torment', 'Torment', 'Torment'], ['BG1EE', 'Baldur', 'Baldur', 'Baldur'], ['BG2EE', 'Baldur', 'Baldur', 'Baldur']]
	If StringRegExp($g_Flags[14], 'BWS|BWP') Then
		$Type = 'BG2'
	Else
		$Type = $g_Flags[14]
	...
	EndIf
	$g_Flags[0]=1
	GUICtrlSetData($g_UI_Interact[6][4], StringFormat(_GetSTR($Message, 'H1'), $Game[$g][0], $Game[$g][2])); => help
	GUICtrlSetData($g_UI_Static[6][1], _GetTR($Message, 'L1')); => watch progress
	GUICtrlSetData($g_UI_Static[6][2], _GetTR($Message, 'L2')); => checking files
	_Process_SwitchEdit(1, 0)
	_Misc_SetTab(6)
	If $MultiDir = $g_GameDir Then; same directory was selected
	...
	ElseIf $Test = 1 Then; installed mods, but has a backup
		_Process_SetScrollLog(@CRLF&_GetTR($Message, 'L7'), 1, -1); => need to restore. Want to?
		_Process_Question('y|n', _GetTR($Message, 'L5'), _GetTR($Message, 'Q1')); => enter yes/no
		If $g_pQuestion = 'y' Then
			$Error = _Backup_Restore(6)
			If $Error = -1 Then; restore was interrupted
				$Error = 1
			ElseIf $Error Then; restore failed
				_Process_SetScrollLog(_GetTR($Message, 'L11'), 1, -1); => restore: fail. Look at log.
			EndIf
		Else; restore was interrupted
			$Error=1
		EndIf
	ElseIf $Test = 3 Then; needs a reinstall
		_Process_SetScrollLog(StringFormat(_GetTR($Message, 'L8'), _GetGameName()), 1, -1); => no backup, install from scratch
		$Error = 1
	EndIf
	If $Error Then
	...
	If $Error = 0 Then; all right
		If Not StringInStr($g_Flags[14], 'EE') Then; alias-section does not seem to exist in BG1EE/BG2EE
		...
		GUICtrlSetData($g_UI_Interact[2][2], $MultiDir); set dir in folder-selection-screen
		IniWrite($g_UsrIni, 'Options', $Type, $MultiDir); write config into current dir
		GUICtrlSetData($g_UI_Interact[6][1], 100)
	...
	EndIf
	_Process_Gui_Delete(3, 3, 1)
EndFunc   ;==>_Backup_CreateMultiInstall
...
Func _Backup_Test($p_Game)
	Local $IsInstalled=0
	Local $Game[8][2]=[[7], ['BG1', 'BGMain2'],['BG2', 'BGMain'], ['IWD1', 'IDMain'],['IWD2', 'IWD2'], ['PST', 'Torment'], ['BG1EE', 'Baldur'], ['BG2EE', 'Baldur']]
	For $g=1 to $Game[0][0]
